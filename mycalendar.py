...
class MyCalendar(calendar.LocaleHTMLCalendar):
    ...
    def formatweek(self, theweek: List[Tuple[int, int]], theyear: int,
                   themonth: int) -> str:
        """
        formatweek
        オーバーライド
    ...
    def formatday(self, day: int, weekday: int, theyear: int,
                  themonth: int) -> str:
        """
        formatday
        """
        ...
        return html.format(url='/todo/{}/{}/{}/{}'.format(
            self.username, theyear, themonth, day),
                           text=text,
                           day=day,
                           highlight=highlight)
